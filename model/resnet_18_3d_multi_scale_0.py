...
# =====================================================================
# 核心逻辑：Layer3(256) 和 Layer4(512) 特征图拼接成 768 维后，降维为 256 维
# =====================================================================
...
class ResNet18_3D(nn.Module):
    def __init__(self, num_classes=2, dropout_rate=0.5):
        ...
        # --- 修正点：根据你的要求，1x1x1 卷积直接将通道数从 768 降维至 256 ---
        self.reduce_planes = 256
    ...
    def forward(self, x):
        ...
        # 4. 通过 1x1x1 卷积完成 768 -> 256 的降维 -> 尺寸变为: [B, 256, 19, 10, 10]
        fused_feat = self.reduce_conv(fused_feat)
        fused_feat = self.reduce_bn(fused_feat)
        fused_feat = self.reduce_relu(fused_feat)
        
        # 5. 分类头处理
        x = self.avgpool(fused_feat)  # 变为 [B, 256, 1, 1, 1]
        x = torch.flatten(x, 1)       # 变为 [B, 256]
        x = self.dropout(x)
