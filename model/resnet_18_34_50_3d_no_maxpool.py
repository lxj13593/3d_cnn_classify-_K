...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=2, dropout_rate=0.5):
        ...
        # 去掉 early maxpool，避免在进入 layer1 前过早压缩 D/长度方向特征
        # 原版这里是 MaxPool3d(kernel_size=(3,3,3), stride=(2,1,1), padding=(1,1,1))
        # 输入 D 方向原来: 296 -> 148 -> 74；现在: 296 -> 148
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # no-maxpool 版本这里不再下采样
