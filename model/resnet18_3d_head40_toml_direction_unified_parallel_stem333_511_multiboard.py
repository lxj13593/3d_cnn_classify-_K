...
class ParallelStem333511(nn.Module):
    """Parallel local-3D and depth-axis stem."""
    ...
    def __init__(self, in_channels: int = 1, out_channels: int = 64) -> None:
        ...
        self.bn_511 = nn.BatchNorm3d(out_channels)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        local = self.bn_333(self.conv_333(x))
        depth = self.bn_511(self.conv_511(x))
        return local + depth
...
class ResNet18Head40TOMLDirectionUnifiedParallelStem333511(nn.Module):
    """Head40 ResNet18 with parallel 3x3x3 and 5x1x1 stem branches."""
    ...
    def __init__(self, num_classes: int = 2, dropout_rate: float = 0.5) -> None:
        ...
        self.conv1 = ParallelStem333511(in_channels=1, out_channels=64)
...
def resnet18_3d(
    ...
    return ResNet18Head40TOMLDirectionUnifiedParallelStem333511(
...
def architecture_metadata() -> dict[str, object]:
    return {
        "model": "ResNet18Head40TOMLDirectionUnifiedParallelStem333511",
        "block": "BasicBlock3D",
        "layers": [2, 2, 2, 2],
        ...
        "stem_type": "parallel_333_511",
        "stem_branch_kernels": [[3, 3, 3], [5, 1, 1]],
        "stem_branch_paddings": [[1, 1, 1], [2, 0, 0]],
        "stem_branch_batchnorm_before_sum": True,
