...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=2, dropout_rate=0.5):
        ...
        # 修改 maxpool：保留局部最大响应选择，但不再做下采样
        # 目的：避免在进入 layer1 前过早压缩 D/长度方向特征，保护头部微小/模糊缺陷信号
        # 原版 stride=(2,1,1): 296 -> conv1 148 -> maxpool 74
        # 本版 stride=(1,1,1): 296 -> conv1 148 -> maxpool 148
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # stride=1，不再下采样，只做局部最大响应选择
