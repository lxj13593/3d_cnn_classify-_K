...
# =====================================================================
# 核心逻辑：Layer2(128)、Layer3(256) 和 Layer4(512) 特征图先进行
# 可学习尺度加权，再拼接成 896 维，之后降维为 512 维。
# 空间尺寸统一对齐到 Layer2 的分辨率。
# =====================================================================
...
class ResNet18_3D(nn.Module):
    def __init__(self, num_classes=2, dropout_rate=0.5):
        ...
        # --- 新增：三个尺度的可学习权重，分别对应 Layer2、Layer3、Layer4 ---
        # 初始化为 0，经过 softmax 后初始权重为 [1/3, 1/3, 1/3]
    ...
    def forward(self, x):
        ...
        # --- 修改点 5：先计算三个尺度的可学习权重，再进行加权后拼接 ---
        # weights[0]、weights[1]、weights[2] 分别对应 Layer2、Layer3、Layer4
        weights = torch.softmax(self.scale_logits, dim=0)
        fused_feat = torch.cat([
            weights[0] * x2,
            weights[1] * x3_upsampled,
            weights[2] * x4_upsampled
        ], dim=1)
...
if __name__ == "__main__":
    ...
    print(f"初始尺度权重 Layer2/Layer3/Layer4: {torch.softmax(model.scale_logits, dim=0).detach().cpu().numpy()}")
