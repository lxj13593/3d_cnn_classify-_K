...
class ResNet3D(nn.Module):
    def __init__(self, block, layers, num_classes=2, dropout_rate=0.5):
        ...
        # 将原始不可学习的 early maxpool 替换为可学习的下采样卷积。
        # 目的：保持与原版相同的特征图尺寸变化，同时避免固定最大池化过早削弱微小/模糊缺陷响应。
        # D/长度方向仍然从 148 下采样到 74，H/W 方向仍然保持分辨率。
        self.maxpool = nn.Sequential(
            nn.Conv3d(64, 64, kernel_size=(3, 3, 3), stride=(2, 1, 1), padding=(1, 1, 1), bias=False),
            nn.BatchNorm3d(64),
    ...
    def forward(self, x):
        ...
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x)  # 可学习下采样 stem，尺寸变化与原版 maxpool 保持一致
