...
class ResNet18Head40TOMLDirectionUnifiedStem533GAPHWAvgDMaxConcat(nn.Module):
    """Head40-533 ResNet18 with concatenated GAP and HWAvg-DMax pooling."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        gap = torch.flatten(self.avgpool(features), 1)
        hw_average = features.mean(dim=(3, 4))
        depth_max = torch.amax(hw_average, dim=2)
        fused = torch.cat((gap, depth_max), dim=1)
...
def resnet18_3d(
    ...
    return ResNet18Head40TOMLDirectionUnifiedStem533GAPHWAvgDMaxConcat(
...
def architecture_metadata() -> dict[str, object]:
    return {
        "model": "ResNet18Head40TOMLDirectionUnifiedStem533GAPHWAvgDMaxConcat",
        "block": "BasicBlock3D",
        "layers": [2, 2, 2, 2],
        ...
        "classifier_pool": "GAP_concat_HW_Avg_then_D_Max",
        "classifier_pool_branches": ["GAP", "HW_Avg_then_D_Max"],
        "classifier_fusion": "concat",
        "classifier_feature_dim": 1024,
