...
class ResNet18Head40TOMLDirectionUnifiedStem533HWAvgDMax(nn.Module):
    """Head40-533 ResNet18 with H/W average then D maximum pooling."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x = x.mean(dim=(3, 4))       # [B, C, D, H, W] -> [B, C, D]
        x = torch.amax(x, dim=2)     # [B, C, D] -> [B, C]
        x = self.dropout(x)
        return self.fc(x)
...
def resnet18_3d(
    ...
    return ResNet18Head40TOMLDirectionUnifiedStem533HWAvgDMax(
...
def architecture_metadata() -> dict[str, object]:
    return {
        "model": "ResNet18Head40TOMLDirectionUnifiedStem533HWAvgDMax",
        "block": "BasicBlock3D",
        "layers": [2, 2, 2, 2],
        ...
        "classifier_pool": "HW_Avg_then_D_Max",
        "classifier_pool_hw_reduction": "mean",
        "classifier_pool_d_reduction": "max",
