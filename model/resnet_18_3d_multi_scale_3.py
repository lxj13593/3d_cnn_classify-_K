...
# =====================================================================
# 核心逻辑：Layer1(64)、Layer2(128)、Layer3(256) 和 Layer4(512) 特征图
# 拼接成 960 维后，降维为 512 维。空间尺寸统一对齐到 Layer2 的分辨率。
# =====================================================================
...
class ResNet18_3D(nn.Module):
    def __init__(self, num_classes=2, dropout_rate=0.5):
        ...
        # --- 修改点 1：动态计算拼接后的总通道数 (64 + 128 + 256 + 512 = 960) ---
        self.combined_planes = 64 + 128 + 256 + 512

        # --- 修改点 2：1x1x1 卷积的输入通道数变更为 960 ---
    ...
    def forward(self, x):
        ...
        # --- 修改点 3：逐步提取 Layer1, Layer2, Layer3, Layer4 的特征 ---
        x1 = self.layer1(x)   # 尺寸: [B, 64, 74, 37, 37]
        x2 = self.layer2(x1)  # 尺寸: [B, 128, 37, 19, 19]
        x3 = self.layer3(x2)  # 尺寸: [B, 256, 19, 10, 10]
        x4 = self.layer4(x3)  # 尺寸: [B, 512, 10, 5, 5]

        # --- 修改点 4：将 Layer1、Layer3 和 Layer4 的空间尺寸统一到 Layer2 的尺寸 (37x19x19) ---
        target_size = x2.shape[2:]
        x1_resized = F.interpolate(x1, size=target_size, mode='trilinear', align_corners=False)
        ...
        # --- 修改点 5：将四个尺度的特征在通道维度拼接 -> 尺寸: [B, 960, 37, 19, 19] ---
        fused_feat = torch.cat([x1_resized, x2, x3_upsampled, x4_upsampled], dim=1)
