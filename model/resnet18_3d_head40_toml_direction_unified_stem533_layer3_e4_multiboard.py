...
class ResNet18Head40TOMLDirectionUnifiedStem533Layer3E4(nn.Module):
    """Head40-533 with an HWAvgMax-k1-multiscale35-DMax Layer3 branch."""
    ...
    def __init__(self, num_classes: int = 2, dropout_rate: float = 0.5) -> None:
        ...
        self.layer3_projection = nn.Conv1d(512, 128, kernel_size=1, bias=False)
        self.layer3_projection_bn = nn.BatchNorm1d(128)
        self.layer3_depth_conv3 = nn.Conv1d(
            128,
            64,
            kernel_size=3,
            padding=1,
        ...
        self.layer3_depth_bn3 = nn.BatchNorm1d(64)
        self.layer3_depth_conv5 = nn.Conv1d(
            128,
            64,
            kernel_size=5,
            padding=2,
        ...
        self.layer3_depth_bn5 = nn.BatchNorm1d(64)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        layer3_hw_maximum = torch.amax(layer3_features, dim=(3, 4))
        layer3_spatial = torch.cat(
            (layer3_hw_average, layer3_hw_maximum),
            dim=1,
        )
        layer3_projected = self.relu(
            self.layer3_projection_bn(self.layer3_projection(layer3_spatial))
        )
        depth_scale3 = self.relu(
            self.layer3_depth_bn3(self.layer3_depth_conv3(layer3_projected))
        )
        depth_scale5 = self.relu(
            self.layer3_depth_bn5(self.layer3_depth_conv5(layer3_projected))
        )
        layer3_aux = torch.cat((depth_scale3, depth_scale5), dim=1)
...
) -> ResNet18Head40TOMLDirectionUnifiedStem533Layer3E4:
    return ResNet18Head40TOMLDirectionUnifiedStem533Layer3E4(
...
def architecture_metadata() -> dict[str, object]:
    return {
        "model": "ResNet18Head40TOMLDirectionUnifiedStem533Layer3E4",
        "block": "BasicBlock3D",
        "layers": [2, 2, 2, 2],
        ...
        "classifier_pool": "Layer4_GAP_concat_Layer3_HWAvgMax_k1_k35_DMax",
        "classifier_pool_branches": ["Layer4_GAP", "Layer3_HWAvgMax_k1_k35_DMax"],
        "classifier_fusion": "concat",
        "classifier_feature_dim": 640,
        "layer4_feature_dim": 512,
        "layer3_aux_variant": "e4",
        "layer3_aux_input_channels": 512,
        "layer3_aux_projection_channels": 128,
        "layer3_aux_spatial_pool": ["average", "maximum"],
        "layer3_aux_spatial_fusion": "channel_concat",
        "layer3_aux_depth_conv_kernels": [3, 5],
        "layer3_aux_depth_conv_channels": [64, 64],
