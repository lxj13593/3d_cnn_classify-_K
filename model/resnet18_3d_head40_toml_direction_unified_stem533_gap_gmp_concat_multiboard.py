...
class ResNet18Head40TOMLDirectionUnifiedStem533GAPGMPConcat(nn.Module):
    """Head40-533 ResNet18 with concatenated global average and max pooling."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        gmp = torch.flatten(self.global_maxpool(features), 1)
        fused = torch.cat((gap, gmp), dim=1)
...
def resnet18_3d(
    ...
    return ResNet18Head40TOMLDirectionUnifiedStem533GAPGMPConcat(
...
def architecture_metadata() -> dict[str, object]:
    return {
        "model": "ResNet18Head40TOMLDirectionUnifiedStem533GAPGMPConcat",
        "block": "BasicBlock3D",
        "layers": [2, 2, 2, 2],
        ...
        "classifier_pool": "GAP_concat_GMP",
        "classifier_pool_branches": ["GAP", "GMP"],
