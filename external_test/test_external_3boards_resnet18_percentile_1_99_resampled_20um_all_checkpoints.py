...
MODEL_NAME = "resnet18_full_percentile_1_99_resampled_20um"
EXPERIMENT_NAME = "resnet18_5fold_ce_percentile_1_99"
CHECKPOINT_FILES = {
    "best_f1": "best_resnet18_3d_percentile_1_99.pth",
    "best_loss": "best_loss_resnet18_3d_percentile_1_99.pth",
    "last": "last_resnet18_3d_percentile_1_99.pth",
}
EXTERNAL_DATA_ROOT = PROJECT_ROOT / "datasets" / "external_test_20um"
DEFAULT_OUTPUT_DIR = (
    TEST_RESULT_DIR
    / "external_3boards_resnet18_percentile_1_99_resampled_20um_all_checkpoints"
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate the per-sample P1/P99 full-volume ResNet18 on three "
            "external boards that were pre-resampled to 20 um spacing."
        )
    )
    parser.add_argument(
        "--data-root",
        ...
        help="Directory containing pre-resampled 20 um external board folders.",
    )
    parser.add_argument(
        "--model-root",
        type=Path,
...
class PercentileExternalBoardDataset(Dataset):
    """Read one variable-shape RAW sample and normalize it independently."""
    ...
    def __init__(self, board_dir, dataset_kind, norm_params):
        ...
        invalid_shapes = [
            ...
            if tuple(sample.input_shape[1:]) != (37, 37)
        ]
        if invalid_shapes:
            raise ValueError(
                f"Full 20 um data must have HxW=37x37: {self.board_dir}; "
...
def validate_inputs(args):
    ...
    if not summary_path.is_file():
        problems.append(f"Missing 20 um resampling summary: {summary_path}")
    else:
        summary = json.loads(summary_path.read_text(encoding="utf-8"))
        if summary.get("dataset_kind") != "full":
            problems.append(f"Invalid full-data summary: {summary_path}")
        if summary.get("target_spacing_um") != 20:
            problems.append(f"Full data is not marked as 20 um: {summary_path}")
...
def main():
    ...
    print(f"Pre-resampled 20 um full data: {args.data_root}")
    ...
    print("Model: full-volume ResNet18 with per-sample P1/P99 normalization.")
    print("Checkpoint policy: best F1, best loss and last epoch are evaluated separately.")
    print("Testing reads pre-resampled 20 um RAW files and performs no resampling.")
    print("Every external sample independently uses P1/P99 normalization.")
    print("Variable-shape testing uses batch_size=1.")
