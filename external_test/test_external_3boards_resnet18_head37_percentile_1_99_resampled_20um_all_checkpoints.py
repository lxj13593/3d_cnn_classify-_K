...
from data_operate.data_load_percentile_1_99 import (  # noqa: E402
    normalize_sample_percentile,
...
MODEL_NAME = "resnet18_head37_percentile_1_99_resampled_20um"
EXPERIMENT_NAME = "resnet18_head_region_37_5fold_ce_percentile_1_99"
CHECKPOINT_FILES = {
...
DEFAULT_OUTPUT_DIR = (
    TEST_RESULT_DIR
    / "external_3boards_resnet18_head37_percentile_1_99_resampled_20um_all_checkpoints"
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate the per-sample P1/P99 Head37 ResNet18 on three external "
            "boards whose prepared head ROIs were pre-resampled to 37x37x37."
        )
    )
    parser.add_argument(
        "--data-root",
...
class PercentileExternalBoardDataset(Dataset):
    """Read one prepared Head37 RAW sample and normalize it independently."""

    def __init__(self, board_dir, dataset_kind, norm_params):
    ...
    def __getitem__(self, index):
        ...
        lower, upper = sample_percentile_bounds(volume)
        tensor = torch.from_numpy(volume.copy()).float().unsqueeze(0)
        tensor = normalize_sample_percentile(tensor, lower, upper)
...
def get_external_loader(board_dir, dataset_kind, norm_params, num_workers=0):
...
def load_model_and_norm(path, device):
    ...
    try:
        ...
        raise ValueError(
            f"Checkpoint does not use per-sample P1/P99 normalization: {path}"
        ) from exc
...
def main():
    ...
    print("Model: Head37 ResNet18 with per-sample P1/P99 normalization.")
    ...
    print("Testing reads the resulting prepared RAW files only.")
    print("No cropping or resampling occurs during testing.")
    print("Every prepared Head37 sample independently uses P1/P99 normalization.")
