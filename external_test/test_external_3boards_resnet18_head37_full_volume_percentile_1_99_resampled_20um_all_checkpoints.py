...
MODEL_NAME = "resnet18_head37_full_volume_percentile_1_99_resampled_20um"
EXPERIMENT_NAME = (
...
EXTERNAL_DATA_ROOT = PROJECT_ROOT / "datasets" / "external_test_head37_20um"
EXTERNAL_FULL_DATA_ROOT = PROJECT_ROOT / "datasets" / "external_test_20um"
DEFAULT_OUTPUT_DIR = (
    TEST_RESULT_DIR
    / "external_3boards_resnet18_head37_full_volume_percentile_1_99_resampled_20um_all_checkpoints"
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate the Head37 ResNet18 that normalizes each existing ROI "
            "with P1/P99 from its corresponding full volume on three boards."
        )
    )
    parser.add_argument(
        "--data-root",
        ...
        help="Directory containing pre-resampled 20 um Head37 data.",
    )
    parser.add_argument(
        "--full-data-root",
        type=Path,
        default=EXTERNAL_FULL_DATA_ROOT,
        help="Directory containing corresponding pre-resampled full 20 um RAW data.",
    )
    parser.add_argument(
        "--model-root",
        type=Path,
...
class PercentileExternalBoardDataset(Dataset):
    """Normalize each existing Head37 crop using its full volume's P1/P99."""
    ...
    def __init__(self, board_dir, full_board_dir, dataset_kind, norm_params):
        ...
        invalid_shapes = [
            ...
            if tuple(sample.input_shape) != (37, 37, 37)
        ]
        if invalid_shapes:
            raise ValueError(
                f"Head37 20 um data must be 37x37x37: {self.board_dir}; "
                f"found {sorted(set(invalid_shapes))}"
            )

        manifest_path = self.board_dir / "manifest_head.csv"
        manifest = read_csv_by_key(manifest_path, "PreparedRawFile")
        self.full_volume_info = []
        for sample in self.samples:
            row = manifest.get(sample.raw_path.name)
            if row is None:
                raise KeyError(
                    f"{sample.raw_path.name} is missing from {manifest_path}"
                )

            full_file_name = row.get("TargetFullRawFile", "").strip()
            if not full_file_name:
                raise ValueError(
                    f"TargetFullRawFile is missing for {sample.raw_path.name}"
                )
            full_shape = tuple(
                int(value) for value in row.get("TargetFullShape", "").split("x")
            )
            if full_shape != FULL_VOLUME_SHAPE:
                raise ValueError(
                    f"Expected full 20 um shape {FULL_VOLUME_SHAPE}, got "
                    f"{full_shape} for {sample.raw_path.name}"
                )

            class_directory = FULL_CLASS_DIRECTORY_BY_LABEL[sample.label]
            full_path = self.full_board_dir / class_directory / full_file_name
            if not full_path.is_file():
                raise FileNotFoundError(
                    "Prepared Head37 sample has no corresponding full 20 um RAW: "
...
def validate_inputs(args):
    ...
    problems = []
    summary_path = args.data_root / "resampling_summary.json"
    if not summary_path.is_file():
        problems.append(f"Missing Head37 resampling summary: {summary_path}")
    else:
        summary = json.loads(summary_path.read_text(encoding="utf-8"))
        if summary.get("dataset_kind") != "head37":
            problems.append(f"Invalid Head37 summary: {summary_path}")
        if summary.get("target_spacing_um") != 20:
            problems.append(f"Head37 data is not marked as 20 um: {summary_path}")
        if summary.get("processing_order") != "resample full volume, then crop Head37":
            problems.append(
                "Head37 data was not generated by resampling the full volume "
                f"before cropping: {summary_path}"
            )
        if not set(args.boards).issubset(set(summary.get("boards", []))):
            problems.append(f"Requested boards are absent from {summary_path}")

    full_summary_path = args.full_data_root / "resampling_summary.json"
    if not full_summary_path.is_file():
        problems.append(f"Missing full-volume resampling summary: {full_summary_path}")
    else:
        full_summary = json.loads(full_summary_path.read_text(encoding="utf-8"))
        if full_summary.get("dataset_kind") != "full":
            problems.append(f"Invalid full-volume summary: {full_summary_path}")
        if full_summary.get("target_spacing_um") != 20:
            problems.append(
                f"Full-volume data is not marked as 20 um: {full_summary_path}"
            )
        if not set(args.boards).issubset(set(full_summary.get("boards", []))):
            problems.append(
                f"Requested boards are absent from {full_summary_path}"
            )
...
def load_model_and_norm(path, device):
    ...
    try:
        ...
        raise ValueError(
            "Checkpoint does not normalize Head37 with corresponding full-volume "
            f"per-sample P1/P99 bounds: {path}"
        ) from exc
...
def main():
    ...
    print(f"Pre-resampled 20 um Head37 data: {args.data_root}")
    print(f"Corresponding full 20 um data: {args.full_data_root}")
    ...
    print(
        ...
        "from the corresponding full volume."
    )
    print("Checkpoint policy: best F1, best loss and last epoch are evaluated separately.")
    print("Head37 data was cropped after full-volume resampling to 20 um.")
    print("Testing reads the existing Head37 crop and its corresponding full RAW.")
    print("No cropping or resampling occurs during testing.")
    print("P1/P99 is calculated from the full RAW, then applied to the Head37 crop.")
