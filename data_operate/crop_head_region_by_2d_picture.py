...
OUT_DATA_DIR = DATASETS_DIR / "head_96" / "data"
OUT_5FOLD_DIR = DATASETS_DIR / "head_96" / "data_5fold"
OUT_LINE_DIR = DATASETS_DIR / "head_96" / "check_images"
...
def crop_raw(raw_path, output_path, location):
    ...
    if volume.size != expected:
        raise ValueError(f"Unexpected raw size: {raw_path}, got {volume.size}, expected {expected}")

    volume = volume.reshape(VOLUME_SHAPE)
    crop = volume[location["crop_start"]:location["crop_end_exclusive"], :, :]
    if location["left_pad"] > 0:
        pad = np.zeros((location["left_pad"], VOLUME_SHAPE[1], VOLUME_SHAPE[2]), dtype=RAW_DTYPE)
...
def iter_raw_files(root_dir):
    for raw_path in sorted(root_dir.rglob("*.raw")):
        if OUT_DATA_DIR in raw_path.parents or OUT_5FOLD_DIR in raw_path.parents:
            continue
        yield raw_path
...
def crop_flat_data(locations, metadata_rows):
    ...
    for class_name in CLASS_NAMES:
        ...
        for raw_path in sorted(class_dir.glob("*.raw")):
            ...
            rel_path = raw_path.relative_to(RAW_DATA_DIR)
            out_path = OUT_DATA_DIR / rel_path
            crop_raw(raw_path, out_path, locations[drill_id])
            metadata_rows.append(make_metadata_row(drill_id, raw_path, out_path, locations[drill_id], "data"))
            count += 1
    return count
...
def crop_5fold_data(locations, metadata_rows):
    ...
    for raw_path in iter_raw_files(RAW_5FOLD_DIR):
        ...
        if drill_id not in locations:
            raise KeyError(f"No picture location for {raw_path.name}")
        rel_path = raw_path.relative_to(RAW_5FOLD_DIR)
        out_path = OUT_5FOLD_DIR / rel_path
        crop_raw(raw_path, out_path, locations[drill_id])
        metadata_rows.append(make_metadata_row(drill_id, raw_path, out_path, locations[drill_id], "data_5fold"))
        count += 1
    return count
...
def draw_all_check_images(locations):
    ...
    for picture_path in sorted(PICTURE_DIR.glob("*.jpg")):
        ...
        if drill_id not in locations:
            continue
        draw_check_image(picture_path, OUT_LINE_DIR / f"{picture_path.stem}.png", locations[drill_id])
        count += 1
    return count
