...
SOURCE_SPLIT_NAME = "resnet18_multiboard_final_5fold"
ORIENTATION_TRAIN_DATASET_NAME = "multiboard_head40_toml"
ORIENTATION_TEST_DATASET_NAME = "test_3boards_head40_toml"
ORIENTATION_TRAIN_ROOT_ENV = "DRILL_HEAD40_TOML_TRAIN_ROOT"
ORIENTATION_TEST_ROOT_ENV = "DRILL_HEAD40_TOML_TEST_ROOT"
...
def is_orientation_dataset_root(path: Path, expected_name: str) -> bool:
...
def resolve_orientation_dataset_root(
    ...
    def validate(candidate: Path) -> Path:
        ...
        if not is_orientation_dataset_root(candidate, expected_name):
            raise FileNotFoundError(
                f"Not the expected orientation dataset '{expected_name}': {candidate}"
            )
        return candidate
    ...
    if is_orientation_dataset_root(project_candidate, expected_name):
        return project_candidate.resolve()
    candidates = []
    for drive_root in mounted_drive_roots():
        candidate = drive_root / DATASET_CONTAINER_RELATIVE_PATH / expected_name
        if is_orientation_dataset_root(candidate, expected_name):
    ...
    raise RuntimeError(
        "Multiple matching orientation datasets were found:\n  "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Independently create full-volume direction-unified five-fold "
            "manifests while preserving the locked baseline assignments."
        )
    )
    parser.add_argument("--source-split-root", type=Path, default=DEFAULT_SOURCE_SPLIT_ROOT)
    parser.add_argument("--orientation-train-dataset-root", type=Path, default=None)
    parser.add_argument("--orientation-test-dataset-root", type=Path, default=None)
...
def main() -> None:
    args = parse_args()
    orientation_train_root = resolve_orientation_dataset_root(
        args.orientation_train_dataset_root,
        ORIENTATION_TRAIN_DATASET_NAME,
        ORIENTATION_TRAIN_ROOT_ENV,
    )
    orientation_test_root = resolve_orientation_dataset_root(
        args.orientation_test_dataset_root,
        ORIENTATION_TEST_DATASET_NAME,
        ORIENTATION_TEST_ROOT_ENV,
    )
    build_direction_unified_splits(
        source_split_root=args.source_split_root,
        orientation_train_root=orientation_train_root,
        orientation_test_root=orientation_test_root,
        output_root=args.output_root,
        volume_kind="full_volume",
