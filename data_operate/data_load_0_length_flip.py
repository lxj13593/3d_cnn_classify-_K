...
try:
    ...
    # 当直接运行 data_load_0_length_flip.py 时
...
class RawCTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # --- 1. 几何增强：横截面翻转/旋转/平移及长度翻转，在归一化之前执行 ---
        if self.augment:
            # 策略：统一逻辑，不再区分 target 0 和 1
            # 目的：保证缺陷特征不被破坏，同时增加数据多样性

            # 1. 随机横截面翻转：从 H/W 两个方向中随机选择一个
            ...
            # 3. 以 50% 概率翻转长度轴 D；张量布局为 [C, D, H, W]
            if torch.rand(1).item() < LENGTH_FLIP_PROBABILITY:
                data = torch.flip(data, dims=[1])

        # 4. 横截面随机平移：所有深度切片使用相同位移
        if self.augment and torch.rand(1).item() > 0.5:
            # 在 X 和 Y 方向随机平移 -3 到 +3 个像素 (约占 37 的 10% 以内)
            shift_y = torch.randint(-3, 4, (1,)).item()
            shift_x = torch.randint(-3, 4, (1,)).item()

            # 使用 zero padding 后裁回原尺寸，避免循环平移引入不真实的边界内容
