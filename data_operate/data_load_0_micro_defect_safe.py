...
class RawCTDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            # 2. 随机旋转：仅在H-W平面；如果缺陷方向有物理意义，可进一步关闭此项
        ...
        if self.augment and torch.rand(1).item() < 0.30:
            # 在 X 和 Y 方向做轻微随机平移。微小缺陷可能只有1~2个像素，
            # 原来的 -3~+3 对 37x37 来说偏强，这里改为 -1~+1。
        ...
        if self.augment:
            # 1. 随机添加高斯噪声：微小/模糊缺陷容易被噪声淹没，强度调低
            if torch.rand(1).item() < 0.30:
                noise_std = 0.003 + torch.rand(1).item() * 0.007  # 随机噪声强度 [0.003, 0.010]
                noise = torch.randn_like(data) * noise_std
                data = data + noise
                data = torch.clamp(data, 0.0, 1.0)

            # 2. 随机亮度调整：范围收窄，避免弱缺陷对比被压低
            if torch.rand(1).item() < 0.30:
                brightness_factor = 0.95 + torch.rand(1).item() * 0.10  # 亮度因子 [0.95, 1.05]
                data = data * brightness_factor
                data = torch.clamp(data, 0.0, 1.0)

            # 3. 随机对比度调整：范围收窄，避免进一步削弱模糊缺陷
            if torch.rand(1).item() < 0.30:
                contrast_factor = 0.95 + torch.rand(1).item() * 0.10  # 对比度因子 [0.95, 1.05]
                # 对比度调整：以0.5为中心进行缩放
