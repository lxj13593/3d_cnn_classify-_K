...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Pre-crop proportional 29^3/37^3 head ROIs for external boards."
    )
    parser.add_argument("--source-root", type=Path, default=DEFAULT_SOURCE_ROOT)
    parser.add_argument("--output-root", type=Path, default=DEFAULT_OUTPUT_ROOT)
    parser.add_argument(
        "--boards", nargs="+", default=["1022", "1077", "1305"]
    )
...
def main():
    ...
    for board in args.boards:
        shape_counts, label_counts, metadata_path = prepare_board(
            source_root, output_root, board
        )
        print(
            f"Board {board}: shapes={shape_counts}, labels={label_counts}, "
