...
from data_load_resnet18_head40_toml_multiboard import (
    CLASS_TO_INDEX,
    DEFECT_DIR_NAME,
    NORMAL_DIR_NAME,
...
DEFAULT_OUTPUT_ROOT = (
    PROJECT_ROOT / "datasets" / "resnet18_head40_toml_multiboard_5fold"
)
BASELINE_SPLIT_ROOT = PROJECT_ROOT / "datasets" / "resnet18_multiboard_final_5fold"
N_SPLITS = 5
SPLIT_SEED = 42
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Create TOML Head40 manifests while preserving the full-volume "
            "baseline's exact within-board five-fold sample assignments."
        )
    )
...
def load_dataset_samples(
    ...
    for row_number, row in enumerate(source_rows, start=2):
        ...
        full_volume_shape_whd = parse_shape(row["FullVolumeRawShape"].strip())
        if shape_whd[2] != 40:
            raise ValueError(f"Expected Head40 depth in row {row_number}: {shape_whd}")
...
def verify_baseline_fold_assignment(
    samples: list[dict[str, object]],
) -> bool | None:
    reference_path = BASELINE_SPLIT_ROOT / "all_samples_with_validation_fold.csv"
    if not reference_path.is_file():
        return None
    reference_rows = read_csv(reference_path)
    reference = {
        row["sample_id"].strip(): int(row["validation_fold"])
        for row in reference_rows
    }
    current = {
        str(row["sample_id"]): int(row["validation_fold"])
        for row in samples
    }
    if set(reference) != set(current):
        ...
        raise ValueError(
            "Head40/baseline SampleID sets differ: "
            f"missing={missing[:5]}, extra={extra[:5]}"
        )
    mismatches = [
        sample_id
        for sample_id in sorted(current)
        if current[sample_id] != reference[sample_id]
    ]
    if mismatches:
        raise ValueError(
            "Head40 fold assignment differs from the baseline: "
            f"{mismatches[:5]}"
        )
    return True
...
def main() -> None:
    ...
    baseline_assignment_verified = verify_baseline_fold_assignment(train_samples)
    ...
    config = {
        ...
        "assignment": "inherited_from_full_volume_by_sample_id",
        "assignment_method": (
            "same deterministic board-label-full_volume_shape algorithm and seed"
        ),
        "baseline_assignment_reference_verified": baseline_assignment_verified,
        "train_fraction": 0.8,
    ...
    print("TOML Head40 multi-board five-fold manifests created")
    ...
    print(f"Baseline fold assignment verified: {baseline_assignment_verified}")
