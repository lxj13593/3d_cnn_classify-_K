"""Head40 multi-board loader for the clear-only fuzzy-weight-0.0 ablation.

The direction-unified Head40 loading pipeline is unchanged.  Each item adds an
``ambiguous`` flag from the manifest's ``difficulty`` field, so the training
entry point can exclude only ``fuzzy`` samples from the supervised objective.
Labels remain normal/defective binary labels.
"""
...
# Re-export the unchanged preprocessing and split protocol.  This prevents data
# handling from drifting relative to the Head40-533 baseline and weight-0.5 run.
...
def is_ambiguous_difficulty(difficulty: str) -> bool:
    """Return whether a source difficulty is a fuzzy, zero-weighted sample."""
