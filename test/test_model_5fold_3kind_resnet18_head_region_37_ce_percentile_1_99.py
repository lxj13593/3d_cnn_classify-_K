...
# 导入自定义模块
from data_operate.data_load_percentile_1_99 import (
    get_test_loader,
...
def main():
    ...
    print(f"\n使用设备: {device}")
    print("=" * 80)
    print("开始执行 ResNet18 + CE 头部区域 5-fold 测试")
    print("=" * 80)
    ...
    for fold_idx, seed in enumerate(fold_seeds):
        ...
        for model_key, file_name in model_files.items():
            ...
            norm_params = validate_normalization_params(
                checkpoint.get('norm_params')
            )
            print(f"Percentile normalization metadata: {norm_params}")
