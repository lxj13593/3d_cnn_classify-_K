...
from model.resnet18_3d_head40_toml_direction_unified_multiboard import (  # noqa: E402
    resnet18_3d,
)


EXPERIMENT_NAME = "resnet18_head40_toml_direction_unified_multiboard_22boards_5fold"
DEFAULT_SPLIT_ROOT = (
    PROJECT_ROOT
    / "datasets"
...
CHECKPOINT_FILES = {
    "best_f1": "best_f1_resnet18_head40_toml_direction_unified.pth",
    "best_loss": "best_loss_resnet18_head40_toml_direction_unified.pth",
    "last": "last_resnet18_head40_toml_direction_unified.pth",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all five direction-unified TOML Head40 ResNet18 folds on "
            "the locked three-board Head40 test set. Checkpoint kinds remain separate."
        )
    )
...
def load_checkpoint_model(
    ...
    if (
        model_metadata.get("model") != "ResNet18Head40TOMLDirectionUnified"
        or model_metadata.get("direction_standardized") is not True
        or model_metadata.get("standardized_head_side") != "high_depth_index"
    ):
        raise ValueError(f"Checkpoint is not the unified-direction Head40 model: {path}")
...
def main() -> None:
    ...
    print("=" * 100)
    print("TOML Head40 direction-unified 3D ResNet18: locked three-board test")
