...
# 导入自定义模块
from data_operate.data_load_0_length_flip import RawCTDataset, compute_global_stats
...
def main():
    ...
    print(f"\n使用设备: {device}")
    print("=" * 80)
    print("开始执行 ResNet18 + CE + Length Flip 5-fold 测试")
    print("=" * 80)
    ...
    for fold_idx, seed in enumerate(fold_seeds):
        ...
        for model_key, file_name in model_files.items():
            ...
            model_path = str(MODEL_BEST_LAST_DIR / EXPERIMENT_NAME / f'fold_{fold_idx}' / file_name)
            save_dir = str(TEST_RESULT_DIR / EXPERIMENT_NAME / f'fold_{fold_idx}' / model_key)
            ...
            print("模型加载成功!")
            if 'augmentation' in checkpoint:
                print(f"训练增强配置: {checkpoint['augmentation']}")
    ...
    if all_test_results:
        ...
        summary_root = str(TEST_RESULT_DIR / EXPERIMENT_NAME)
