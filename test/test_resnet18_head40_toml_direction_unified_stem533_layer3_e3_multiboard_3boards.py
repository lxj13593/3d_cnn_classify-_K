...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all five Head40-533 Layer3 E3 HWAvg-k1-multiscale35-DMax "
...
def load_checkpoint_model(
    ...
    if model_metadata != architecture_metadata():
        raise ValueError(f"Checkpoint Layer3 E3 architecture metadata mismatch: {path}")
    if (
        model_metadata.get("layer3_aux_variant") != "e3"
        or model_metadata.get("classifier_feature_dim") != 640
    ):
        raise ValueError(f"Checkpoint is not the required Layer3 E3 model: {path}")
...
def main() -> None:
    ...
    print("=" * 100)
    print("Head40-533 Layer3 E3 HWAvg-k1-multiscale35-DMax: locked three-board test")
