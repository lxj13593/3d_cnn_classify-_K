...
# 添加项目根目录，以便导入 data_operate 和 model 模块
...
def get_test_loader(data_root, shape=HEAD_REGION_VOLUME_SHAPE, dtype=RAW_DTYPE, batch_size=BATCH_SIZE, num_workers=NUM_WORKERS,
...
def main():
    ...
    model_files = HEAD_REGION_MODEL_FILES

    all_test_results = []
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n使用设备: {device}")
    print("=" * 80)
    print("开始执行 ResNet18 + CE 头部24区域保留长度 5-fold 测试")
    print("=" * 80)
    ...
    for fold_idx, seed in enumerate(fold_seeds):
        ...
        for model_key, file_name in model_files.items():
            ...
            model_path = str(MODEL_BEST_LAST_DIR / HEAD_REGION_EXPERIMENT / f'fold_{fold_idx}' / file_name)
            save_dir = str(TEST_RESULT_DIR / HEAD_REGION_EXPERIMENT / f'fold_{fold_idx}' / model_key)
    ...
    if all_test_results:
        ...
        summary_root = str(TEST_RESULT_DIR / HEAD_REGION_EXPERIMENT)
