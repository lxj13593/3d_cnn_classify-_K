...
from model.resnet18_3d_multiboard_direction_unified import resnet18_3d  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all five full-volume direction-unified ResNet18 folds on "
            "the locked three-board test set. Checkpoint kinds remain separate."
        )
    )
...
def load_checkpoint_model(
    ...
    if (
        model_metadata.get("model") != "ResNet18DirectionUnified"
        or model_metadata.get("direction_standardized") is not True
        or model_metadata.get("standardized_head_side") != "high_depth_index"
    ):
        raise ValueError(f"Checkpoint is not the unified-direction full model: {path}")
    ...
    try:
        ...
        with pd.ExcelWriter(path) as writer:
            ...
            for board, group in frame.groupby("Board", sort=False):
                sheet = "ALL" if board == "ALL" else str(board)[8:23]
                group.drop(columns=["Board"]).to_excel(writer, sheet_name=sheet[:31], index=False)
...
def validate_inputs(args: argparse.Namespace, test_manifest: Path) -> None:
    if not test_manifest.is_file():
        raise FileNotFoundError(
            ...
            "make_resnet18_multiboard_direction_unified_5fold.py first."
        )
    missing = []
...
def main() -> None:
    ...
    print("=" * 100)
    print("Full-volume direction-unified 3D ResNet18: locked three-board test")
    print(f"Test dataset: {test_root}")
    print(f"Models: {args.model_root}")
    print(f"Checkpoints: {args.checkpoint_kinds}")
    print(f"Device: {device}; AMP: {amp_enabled}")
    print("Normalization: per-sample full-volume P1/P99, identical to training")
    print("Direction: bHeadUp=true flips D; every drill head is at high D index")
    print("No augmentation, interpolation, cropping, retraining or threshold tuning")
    print("=" * 100)
