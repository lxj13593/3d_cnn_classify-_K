...
from model.resnet18_3d_head40_toml_direction_unified_layer1_block1_conv1_acb_multiboard import (  # noqa: E402
...
EXPERIMENT_NAME = (
    "resnet18_head40_toml_direction_unified_"
    "layer1_block1_conv1_acb_multiboard_22boards_5fold"
)
DEFAULT_SPLIT_ROOT = (
    PROJECT_ROOT
    / "datasets"
...
CHECKPOINT_FILES = {
    "best_f1": "best_f1_resnet18_head40_toml_direction_unified_layer1_block1_conv1_acb.pth",
    "best_loss": "best_loss_resnet18_head40_toml_direction_unified_layer1_block1_conv1_acb.pth",
    "last": "last_resnet18_head40_toml_direction_unified_layer1_block1_conv1_acb.pth",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all five direction-unified TOML Head40 ResNet18 Layer1 "
            "Block1 Conv1 ACB folds on the locked three-board Head40 test set. "
            "Checkpoint kinds remain separate."
        )
    )
...
def load_checkpoint_model(
    ...
    expected_metadata = architecture_metadata()
    if model_metadata != expected_metadata:
        raise ValueError(f"Checkpoint ACB architecture metadata mismatch in {path}")
    if any(
        model_metadata.get(key) != expected
        for key, expected in EXPECTED_ACB_METADATA.items()
    ):
        raise ValueError(f"Checkpoint is not the required Layer1 Block1 Conv1 ACB model: {path}")
...
def validate_inputs(args: argparse.Namespace, test_manifest: Path) -> None:
    if not test_manifest.is_file():
        raise FileNotFoundError(
            f"Missing {test_manifest}. Run "
            "make_resnet18_head40_toml_direction_unified_multiboard_5fold.py first."
        )
    missing = []
...
def main() -> None:
    ...
    print("=" * 100)
    print(
        "TOML Head40 direction-unified 3D ResNet18 "
        "Layer1 Block1 Conv1 ACB: locked three-board test"
    )
