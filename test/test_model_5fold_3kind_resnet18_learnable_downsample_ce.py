...
def main():
    ...
    model_files = RESNET18_LEARNABLE_DOWNSAMPLE_MODEL_FILES

    all_test_results = []
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n使用设备: {device}")
    print("=" * 80)
    print("开始执行 ResNet18 learnable-downsample + CE 5-fold 测试")
    print("=" * 80)
    ...
    for fold_idx, seed in enumerate(fold_seeds):
        ...
        for model_key, file_name in model_files.items():
            ...
            model_path = str(MODEL_BEST_LAST_DIR / RESNET18_LEARNABLE_DOWNSAMPLE_EXPERIMENT / f'fold_{fold_idx}' / file_name)
            save_dir = str(TEST_RESULT_DIR / RESNET18_LEARNABLE_DOWNSAMPLE_EXPERIMENT / f'fold_{fold_idx}' / model_key)
    ...
    if all_test_results:
        ...
        summary_root = str(TEST_RESULT_DIR / RESNET18_LEARNABLE_DOWNSAMPLE_EXPERIMENT)
