...
from data_operate.data_load_resnet18_multiboard_final import (  # noqa: E402
    TEST_DATASET_NAME,
...
from model.resnet18_3d_multiboard import resnet18_3d  # noqa: E402


EXPERIMENT_NAME = "resnet18_multiboard_final_22boards_5fold"
DEFAULT_SPLIT_ROOT = PROJECT_ROOT / "datasets" / "resnet18_multiboard_final_5fold"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all five original ResNet18 folds on the locked three-board "
            "test set. best_f1, best_loss and last are kept separate."
        )
    )
...
def validate_inputs(args: argparse.Namespace, test_manifest: Path) -> None:
    if not test_manifest.is_file():
        raise FileNotFoundError(
            f"Missing {test_manifest}. Run make_resnet18_multiboard_final_5fold.py first."
        )
    missing = []
...
def main() -> None:
    ...
    print("=" * 100)
    print("Original 3D ResNet18: locked three-board independent test")
