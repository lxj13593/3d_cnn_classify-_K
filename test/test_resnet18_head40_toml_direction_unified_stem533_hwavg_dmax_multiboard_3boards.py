...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all five Head40 Stem533 HWAvg-DMax ResNet18 "
...
def load_checkpoint_model(
    ...
    if (
        ...
        != "ResNet18Head40TOMLDirectionUnifiedStem533HWAvgDMax"
        or model_metadata.get("stem_conv_kernel") != [5, 3, 3]
        or model_metadata.get("classifier_pool") != "HW_Avg_then_D_Max"
        or model_metadata.get("direction_standardized") is not True
        or model_metadata.get("standardized_head_side") != "high_depth_index"
    ):
        raise ValueError(
            f"Checkpoint is not the Head40 Stem533 HWAvg-DMax model: {path}"
        )
...
def main() -> None:
    ...
    print("=" * 100)
    print("Head40 Stem533 HWAvg-DMax ResNet18: locked three-board test")
