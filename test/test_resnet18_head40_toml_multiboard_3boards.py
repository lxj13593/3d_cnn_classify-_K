...
from model.resnet18_3d_head40_toml_multiboard import resnet18_3d  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all five TOML Head40 ResNet18 folds on the locked "
            "three-board Head40 test set. best_f1, best_loss and last are "
            "kept separate."
        )
    )
...
def load_checkpoint_model(
    ...
    if model_metadata.get("model") != "ResNet18Head40TOML":
        raise ValueError(f"Checkpoint is not the TOML Head40 model: {path}")
...
def validate_inputs(args: argparse.Namespace, test_manifest: Path) -> None:
    if not test_manifest.is_file():
        raise FileNotFoundError(
            ...
            "make_resnet18_head40_toml_multiboard_5fold.py first."
        )
    missing = []
...
def main() -> None:
    ...
    print("=" * 100)
    print("TOML Head40 3D ResNet18: locked three-board independent test")
