...
try {
    $SplitArgs = @("data_operate\make_resnet18_head40_toml_multiboard_5fold.py")
    if (-not [string]::IsNullOrWhiteSpace($TrainDatasetRoot)) {
        $SplitArgs += @("--train-dataset-root", $TrainDatasetRoot)
    }
    if (-not [string]::IsNullOrWhiteSpace($TestDatasetRoot)) {
        $SplitArgs += @("--test-dataset-root", $TestDatasetRoot)
    }
    & $PythonExe @SplitArgs
    if ($LASTEXITCODE -ne 0) {
        throw "Dataset and leakage audit failed with exit code $LASTEXITCODE"
    }

    $TestArgs = @(
        "test\test_resnet18_head40_toml_multiboard_3boards.py",
