...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all five Head40 Parallel-Stem333-511 folds on the "
            "locked three-board test set."
        )
    )
...
def load_checkpoint_model(
    ...
    if (
        ...
        != "ResNet18Head40TOMLDirectionUnifiedParallelStem333511"
        or model_metadata.get("stem_type") != "parallel_333_511"
        or model_metadata.get("stem_branch_kernels")
        != [[3, 3, 3], [5, 1, 1]]
        or model_metadata.get("direction_standardized") is not True
        or model_metadata.get("standardized_head_side") != "high_depth_index"
    ):
        raise ValueError(
            f"Checkpoint is not the Head40 Parallel-Stem333-511 model: {path}"
        )
...
def main() -> None:
    ...
    print("=" * 100)
    print("Head40 Parallel-Stem333-511 ResNet18: locked three-board test")
