"""Five-fold Head40-533 training with fuzzy samples weighted by 0.5.

The classifier remains normal-versus-defective.  Only fuzzy samples affect the
training objective differently; validation always uses ordinary unweighted CE.
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "weighted at 0.5 in the training CE objective."
        )
    )
...
def train_one_epoch(
    ...
    """Train one epoch using the requested weighted mean of per-sample CE."""
    model.train()
...
def checkpoint_payload(**kwargs) -> dict[str, object]:
    ...
    payload["loss_function"] = "CrossEntropyLoss_per_sample_ambiguous_weight05"
...
def run_fold(
    ...
    run_config = {
        ...
        "loss": "per-sample CrossEntropyLoss with fuzzy weight 0.5",
    ...
    print("Loss: per-sample CE; fuzzy difficulty weight=0.5; primary checkpoint=best_loss")
    print("=" * 100)
...
def main() -> None:
    ...
    print("=" * 100)
    print("Head40-533: fuzzy samples use a 0.5 training-loss weight")
    print(f"Folds: {folds}")
