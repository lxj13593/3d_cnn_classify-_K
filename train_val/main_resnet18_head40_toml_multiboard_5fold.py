...
from data_operate.data_load_resnet18_head40_toml_multiboard import (  # noqa: E402
    CLASS_TO_INDEX,
    ShapeBatchSampler,
    TRAIN_DATASET_NAME,
...
from model.resnet18_3d_head40_toml_multiboard import (  # noqa: E402
    architecture_metadata,
    resnet18_3d,
)


EXPERIMENT_NAME = "resnet18_head40_toml_multiboard_22boards_5fold"
DEFAULT_SPLIT_ROOT = (
...
CHECKPOINT_FILES = {
    "best_f1": "best_f1_resnet18_head40_toml.pth",
    "best_loss": "best_loss_resnet18_head40_toml.pth",
    "last": "last_resnet18_head40_toml.pth",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train the Head40 3D ResNet18 on the TOML-cropped 22-board "
            "dataset using the exact validation-fold assignments inherited "
            "from the full-volume multi-board baseline."
        )
    )
...
def load_split_config(split_root: Path) -> dict[str, object]:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing {path}. Run "
            "data_operate/make_resnet18_head40_toml_multiboard_5fold.py first."
        )
    ...
    if config.get("assignment") != "inherited_from_full_volume_by_sample_id":
        raise ValueError("Head40 split does not inherit the baseline fold assignment")
    expected_counts = {
        "sample_count": 4364,
        "normal_count": 3181,
...
def main() -> None:
    ...
    print("=" * 100)
    print("TOML Head40 3D ResNet18: 22-board inherited 5-fold CV")
    print(f"Folds: {folds}")
    ...
    print("Normalization: per-sample Head40 P1/P99; no length-direction flip.")
    print("=" * 100)
