...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train the original 3D ResNet18 on the final 22-board "
            "multi-board dataset with within-board balanced 5-fold "
...
def load_split_config(split_root: Path) -> dict[str, object]:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing {path}. Run "
            "data_operate/make_resnet18_multiboard_final_5fold.py first."
        )
...
def main() -> None:
    ...
    print("=" * 100)
    print("Original 3D ResNet18: final 22-board 5-fold CV")
    print(f"Folds: {folds}")
