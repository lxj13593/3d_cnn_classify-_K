...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Path to clear_binary_dataset. If omitted, use the current valid "
            "stored path, DRILL_DATASET_ROOT, or automatic drive discovery."
        ),
    )
...
def dataset_integrity_check(train_loader, val_loader, fold: int) -> None:
    ...
    if len({sample.board for sample in train_samples}) != 15:
...
def main() -> None:
    ...
    print("Train/validation = 80%/20%; no internal test; no external test.")
    print("=" * 100)
