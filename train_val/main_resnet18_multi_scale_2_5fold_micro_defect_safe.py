...
from data_operate.data_load_0_micro_defect_safe import get_data_loaders
from model.resnet_18_3d_multi_scale_2 import ResNet18_3D
...
from data_operate.config import (
    ...
    RANDOM_SEED,
    RAW_DTYPE,
    RESNET18_MULTI_SCALE_2_EXPERIMENT,
    RESNET18_MULTI_SCALE_2_MICRO_DEFECT_SAFE_EXPERIMENT,
...
def run_pipeline(fold_idx, seed, num_epochs=EPOCHS_PER_RUN):
    ...
    # 使用 ResNet18-MultiScale-2 + Dropout正则化
    model = ResNet18_3D(num_classes=NUM_CLASSES, dropout_rate=DROPOUT_RATE)
    ...
    model_dir = str(MODEL_BEST_LAST_DIR / RESNET18_MULTI_SCALE_2_MICRO_DEFECT_SAFE_EXPERIMENT / f'fold_{fold_idx}')
    picture_dir = str(TRAIN_VAL_RESULT_DIR / RESNET18_MULTI_SCALE_2_MICRO_DEFECT_SAFE_EXPERIMENT / f'fold_{fold_idx}')
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            # 保存最佳模型
            save_path = os.path.join(model_dir, RESNET18_MULTI_SCALE_2_MODEL_FILES['best_f1'])
            torch.save({
                'epoch': epoch + 1,
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_loss_path = os.path.join(model_dir, RESNET18_MULTI_SCALE_2_MODEL_FILES['best_loss'])
            torch.save({
                'epoch': epoch + 1,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    # 5. 保存最后一轮模型
    last_model_path = os.path.join(model_dir, RESNET18_MULTI_SCALE_2_MODEL_FILES['last'])
    torch.save({
        'epoch': num_epochs,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print(f" 开始执行 ResNet18 + Layer234 多尺度 + CE 5-fold 训练。fold seeds: {fold_seeds} | 单折: {EPOCHS_PER_RUN} 轮")
    print("=" * 70)
    ...
    summary_root = TRAIN_VAL_RESULT_DIR / RESNET18_MULTI_SCALE_2_MICRO_DEFECT_SAFE_EXPERIMENT
