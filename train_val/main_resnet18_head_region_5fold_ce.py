...
from model.resnet_18_34_50_3d_head_region import ResNet3D, BasicBlock
...
HEAD_REGION_DATA_5FOLD_DIR = (
    Path(__file__).resolve().parent.parent / "datasets" / "head_96" / "data_5fold"
)
HEAD_REGION_VOLUME_SHAPE = (96, 37, 37)
HEAD_REGION_EXPERIMENT = "resnet18_head_region_5fold_ce"
HEAD_REGION_MODEL_FILES = {
    "best_f1": "best_resnet18_head_region_3d.pth",
    "best_loss": "best_loss_resnet18_head_region_3d.pth",
    "last": "last_resnet18_head_region_3d.pth",
}
...
def run_pipeline(fold_idx, seed, num_epochs=EPOCHS_PER_RUN):
    ...
    picture_dir = str(TRAIN_VAL_RESULT_DIR / HEAD_REGION_EXPERIMENT / f'fold_{fold_idx}')
    ...
    for epoch in range(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_loss_path = os.path.join(model_dir, HEAD_REGION_MODEL_FILES['best_loss'])
            torch.save({
                'epoch': epoch + 1,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print(f" 开始执行 ResNet18 + CE 头部区域 5-fold 训练。fold seeds: {fold_seeds} | 单折: {EPOCHS_PER_RUN} 轮")
    print("=" * 70)
