...
from model.resnet_18_34_50_3d_layer2_keep37 import ResNet3D, BasicBlock
...
from data_operate.config import (
    ...
    RANDOM_SEED,
    RAW_DTYPE,
    RESNET18_LAYER2_KEEP37_EXPERIMENT,
...
def run_pipeline(fold_idx, seed, num_epochs=EPOCHS_PER_RUN):
    ...
    model_dir = str(MODEL_BEST_LAST_DIR / RESNET18_LAYER2_KEEP37_EXPERIMENT / f'fold_{fold_idx}')
    picture_dir = str(TRAIN_VAL_RESULT_DIR / RESNET18_LAYER2_KEEP37_EXPERIMENT / f'fold_{fold_idx}')
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            # 保存最佳模型
            save_path = os.path.join(model_dir, RESNET18_LAYER2_KEEP37_MODEL_FILES['best_f1'])
            torch.save({
                'epoch': epoch + 1,
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_loss_path = os.path.join(model_dir, RESNET18_LAYER2_KEEP37_MODEL_FILES['best_loss'])
            torch.save({
                'epoch': epoch + 1,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    # 5. 保存最后一轮模型
    last_model_path = os.path.join(model_dir, RESNET18_LAYER2_KEEP37_MODEL_FILES['last'])
    torch.save({
        'epoch': num_epochs,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print(f" 开始执行 ResNet18 layer2-keep37 + CE 5-fold 训练。fold seeds: {fold_seeds} | 单折: {EPOCHS_PER_RUN} 轮")
    print("=" * 70)
    ...
    summary_root = TRAIN_VAL_RESULT_DIR / RESNET18_LAYER2_KEEP37_EXPERIMENT
