...
from data_operate.data_load_percentile_1_99 import get_data_loaders
from model.resnet_18_34_50_3d_head_region_37 import ResNet3D, BasicBlock
...
HEAD_REGION_DATA_5FOLD_DIR = (
    Path(__file__).resolve().parent.parent / "datasets" / "head_37" / "data_5fold"
)
HEAD_REGION_VOLUME_SHAPE = (37, 37, 37)
HEAD_REGION_EXPERIMENT = "resnet18_head_region_37_5fold_ce_percentile_1_99"
HEAD_REGION_MODEL_FILES = {
    "best_f1": "best_resnet18_head_region_37_3d_percentile_1_99.pth",
    "best_loss": "best_loss_resnet18_head_region_37_3d_percentile_1_99.pth",
    "last": "last_resnet18_head_region_37_3d_percentile_1_99.pth",
}
...
def run_pipeline(fold_idx, seed, num_epochs=EPOCHS_PER_RUN):
    ...
    model = ResNet3D(
        BasicBlock, [2, 2, 2, 2], num_classes=NUM_CLASSES, dropout_rate=DROPOUT_RATE
    )
    ...
    data_root = str(HEAD_REGION_DATA_5FOLD_DIR / f"fold_{fold_idx}")
    print(f"\n数据根目录: {data_root}")
    ...
    model_dir = str(MODEL_BEST_LAST_DIR / HEAD_REGION_EXPERIMENT / f"fold_{fold_idx}")
    picture_dir = str(
        TRAIN_VAL_RESULT_DIR / HEAD_REGION_EXPERIMENT / f"fold_{fold_idx}"
    )
    ...
    for epoch in range(num_epochs):
        ...
        train_precision = precision_score(
            all_train_labels, all_train_preds, zero_division=0
        )
        ...
        train_specificity = (
            cm_train[0, 0] / (cm_train[0, 0] + cm_train[0, 1])
            if (cm_train[0, 0] + cm_train[0, 1]) > 0
            else 0.0
        )
        ...
        val_precision = precision_score(
            all_val_labels_list, all_val_preds_list, zero_division=0
        )
        ...
        cm_val = confusion_matrix(
            all_val_labels_list, all_val_preds_list, labels=[0, 1]
        )
        val_specificity = (
            cm_val[0, 0] / (cm_val[0, 0] + cm_val[0, 1])
            if (cm_val[0, 0] + cm_val[0, 1]) > 0
            else 0.0
        )
        ...
        if val_f1 > best_val_f1:
            ...
            # 保存最佳模型
            save_path = os.path.join(model_dir, HEAD_REGION_MODEL_FILES["best_f1"])
            torch.save(
                {
                    "epoch": epoch + 1,
                    ...
                    "norm_params": dict(
                        train_loader.dataset.normalization_params
                    ),
                },
                save_path,
            )
            print(
                f"🎉 新高! 最佳F1模型已保存: {save_path} (F1: {best_val_f1:.4f}, Acc: {val_acc:.2f}%)"
            )
            ...
            disp = ConfusionMatrixDisplay(
                confusion_matrix=cm, display_labels=list(DISPLAY_LABELS)
            )
            fig, ax = plt.subplots(figsize=(8, 6))
            disp.plot(ax=ax, cmap="Blues", values_format="d")
            plt.title(
                f"Confusion Matrix (Best Epoch {epoch + 1}, F1: {best_val_f1:.4f}, Acc: {val_acc:.2f}%)"
            )
            ...
            precision_vals, recall_vals, _ = precision_recall_curve(
                all_val_labels_list, all_val_probs_list
            )
            ...
            plt.plot(
                ...
                color="darkblue",
                lw=2,
                label=f"PR curve (AP = {best_ap:.4f})",
            )
            plt.xlabel("Recall")
            plt.ylabel("Precision")
            plt.title(
                f"Precision-Recall Curve (Best Epoch {epoch + 1}, AP: {best_ap:.4f})"
            )
            ...
            # 更新当前种子的最优核心表现
            best_metrics_summary = {"val_f1": val_f1, "val_acc": val_acc}

        # --- 当验证集 Loss 创新低时，保存最低 Loss 模型 ---
        if avg_val_loss < best_val_loss:
            best_val_loss = avg_val_loss
            save_loss_path = os.path.join(
                model_dir, HEAD_REGION_MODEL_FILES["best_loss"]
            )
            torch.save(
                {
                    "epoch": epoch + 1,
            ...
            print(
                f"📉 损创新低! 最低 Loss 模型已保存: {save_loss_path} (Loss: {best_val_loss:.4f})"
            )
    ...
    # 5. 保存最后一轮模型
    last_model_path = os.path.join(model_dir, HEAD_REGION_MODEL_FILES["last"])
    torch.save(
        {
            "epoch": num_epochs,
            "model_state_dict": model.state_dict(),
            ...
            "val_accuracy": val_acc,
            "train_accuracy": train_acc,
            "norm_params": dict(train_loader.dataset.normalization_params),
        },
        last_model_path,
    )
    ...
    disp = ConfusionMatrixDisplay(
        confusion_matrix=cm_last, display_labels=list(DISPLAY_LABELS)
    )
    fig, ax = plt.subplots(figsize=(8, 6))
    disp.plot(ax=ax, cmap="Blues", values_format="d")
    ...
    plt.plot(
        ...
        color="darkorange",
        lw=2,
        label=f"ROC curve (AUC = {auc_last:.4f})",
    )
    plt.plot([0, 1], [0, 1], color="navy", lw=2, linestyle="--")
    plt.xlim([0.0, 1.0])
    plt.ylim([0.0, 1.05])
    ...
    # 最终模型 PR 曲线
    precision_vals_last, recall_vals_last, _ = precision_recall_curve(
    ...
    plt.plot(
        ...
        color="darkblue",
        lw=2,
        label=f"PR curve (AP = {ap_last:.4f})",
    )
    plt.xlabel("Recall")
    plt.ylabel("Precision")
    ...
    plt.plot(
        ...
        "b--",
        label="Train Loss (running, train mode)",
        linewidth=2,
    )
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(
        range(1, num_epochs + 1), train_accs, "b-", label="Train Accuracy", linewidth=2
    )
    plt.plot(
        range(1, num_epochs + 1), val_accs, "r-", label="Val Accuracy", linewidth=2
    )
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy (%)")
    ...
    plt.figure(figsize=(10, 6))
    plt.plot(
        range(1, num_epochs + 1), train_f1s, "b-", label="Train F1-Score", linewidth=2
    )
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print(
        f" 开始执行 ResNet18 + CE 头部区域 5-fold 训练。fold seeds: {fold_seeds} | 单折: {EPOCHS_PER_RUN} 轮"
    )
    print("=" * 70)
    ...
    for fold_idx, seed in enumerate(fold_seeds):
        ...
        run_summary = run_pipeline(
            fold_idx=fold_idx, seed=seed, num_epochs=EPOCHS_PER_RUN
        )
    ...
    summary_root = TRAIN_VAL_RESULT_DIR / HEAD_REGION_EXPERIMENT
