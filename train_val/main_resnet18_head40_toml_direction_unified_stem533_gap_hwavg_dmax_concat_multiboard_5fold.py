...
from model.resnet18_3d_head40_toml_direction_unified_stem533_gap_hwavg_dmax_concat_multiboard import (  # noqa: E402
    architecture_metadata,
    resnet18_3d,
)

EXPERIMENT_NAME = (
    "resnet18_head40_toml_direction_unified_stem533_gap_hwavg_dmax_concat_multiboard_22boards_5fold"
)
DEFAULT_SPLIT_ROOT = (
    PROJECT_ROOT
    / "datasets"
...
CHECKPOINT_FILES = {
    "best_f1": "best_f1_resnet18_head40_toml_direction_unified_stem533_gap_hwavg_dmax_concat.pth",
    "best_loss": "best_loss_resnet18_head40_toml_direction_unified_stem533_gap_hwavg_dmax_concat.pth",
    "last": "last_resnet18_head40_toml_direction_unified_stem533_gap_hwavg_dmax_concat.pth",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Train Head40 Stem533 ResNet18 with concatenated GAP and HWAvg-DMax pooling and an "
            "anisotropic 5x3x3 stem on the fixed 22-board five-fold manifests."
        )
    )
                ...
                prediction_rows.append(
                    {
                        ...
                        "standardized_head_side": batch["standardized_head_side"][
                            index
                        ],
...
def dataset_integrity_check(train_loader, val_loader, fold: int) -> None:
    ...
    if any(sample.validation_fold != fold for sample in val_samples):
        raise ValueError(
            f"Fold {fold}: validation manifest contains wrong fold samples"
        )
...
def load_split_config(split_root: Path) -> dict[str, object]:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing shared Head40 split config: {path}. "
    ...
    if config.get("internal_test_set") is not False:
        raise ValueError(
            "This experiment requires train/validation folds without test sets"
        )
    ...
    if config.get("assignment") != expected_assignment:
        raise ValueError(
            "Head40 split does not contain the unified-direction assignment"
        )
...
def save_grouped_oof_metrics(
    ...
    for key, rows in sorted(groups.items()):
        metrics = calculate_metrics(
            ...
            average_loss=float(np.mean([float(row["sample_loss"]) for row in rows])),
        )
...
def main() -> None:
    ...
    if args.epochs <= 0 or args.batch_size <= 0 or args.num_workers < 0:
        raise ValueError(
            "epochs/batch-size must be positive and num-workers non-negative"
        )
    ...
    print("=" * 100)
    print("Head40 Stem533 GAP+HWAvg-DMax Concat ResNet18: 22-board 5-fold CV")
    print(f"Folds: {folds}")
    ...
    print("Normalization: per-sample Head40 P1/P99.")
