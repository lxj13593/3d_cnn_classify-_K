...
from data_operate.data_load_head37_full_volume_percentile_1_99 import (
    get_data_loaders,
)
...
FULL_VOLUME_DATA_5FOLD_DIR = (
    Path(__file__).resolve().parent.parent / "datasets" / "original" / "data_5fold"
)
HEAD_REGION_VOLUME_SHAPE = (37, 37, 37)
HEAD_REGION_EXPERIMENT = (
...
def run_pipeline(fold_idx, seed, num_epochs=EPOCHS_PER_RUN):
    ...
    full_data_root = str(FULL_VOLUME_DATA_5FOLD_DIR / f"fold_{fold_idx}")
    print(f"\n数据根目录: {data_root}")
    print(f"完整体积归一化数据根目录: {full_data_root}")
