...
from data_operate.data_load_0_length_flip import LENGTH_FLIP_PROBABILITY, get_data_loaders
...
EXPERIMENT_NAME = "resnet18_5fold_length_flip"
MODEL_FILES = {
    "best_f1": "best_resnet18_3d_length_flip.pth",
    "best_loss": "best_loss_resnet18_3d_length_flip.pth",
    "last": "last_resnet18_3d_length_flip.pth",
}
...
def run_pipeline(fold_idx, seed, num_epochs=EPOCHS_PER_RUN):
    ...
    print(f"\n数据根目录: {data_root}")
    print(f"训练增强: 长度方向随机翻转，概率={LENGTH_FLIP_PROBABILITY:.2f}")
    ...
    # 使用独立实验目录，避免覆盖原版 ResNet18 的权重和结果
    model_dir = str(MODEL_BEST_LAST_DIR / EXPERIMENT_NAME / f'fold_{fold_idx}')
    picture_dir = str(TRAIN_VAL_RESULT_DIR / EXPERIMENT_NAME / f'fold_{fold_idx}')
    ...
    for epoch in range(num_epochs):
        ...
        if val_f1 > best_val_f1:
            ...
            # 保存最佳模型
            save_path = os.path.join(model_dir, MODEL_FILES['best_f1'])
            torch.save({
                'epoch': epoch + 1,
                ...
                'augmentation': {
                    'length_flip_probability': LENGTH_FLIP_PROBABILITY,
                    'length_axis': 'D'
                }
            }, save_path)
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_loss_path = os.path.join(model_dir, MODEL_FILES['best_loss'])
            torch.save({
                'epoch': epoch + 1,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    # 5. 保存最后一轮模型
    last_model_path = os.path.join(model_dir, MODEL_FILES['last'])
    torch.save({
        'epoch': num_epochs,
        'model_state_dict': model.state_dict(),
        'optimizer_state_dict': optimizer.state_dict(),
...
if __name__ == '__main__':
    ...
    print("=" * 70)
    print(f" 开始执行 ResNet18 + CE + Length Flip 5-fold 训练。fold seeds: {fold_seeds} | 单折: {EPOCHS_PER_RUN} 轮")
    print("=" * 70)
    ...
    summary_root = TRAIN_VAL_RESULT_DIR / EXPERIMENT_NAME
